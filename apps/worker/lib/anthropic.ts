...
function parseAnalyzeResponse(response: string): AnalyzeResult {
  ...
  return {
    ...
    issues: (parsed.issues || []).map((issue: Record<string, unknown>) => ({
      ...
      zone: {
        x: Math.min(100, Math.max(0, Number(issue.zone?.x) || 0)),
        y: Math.min(100, Math.max(0, Number(issue.zone?.y) || 0)),
        width: Math.min(100, Math.max(0, Number(issue.zone?.width) || 10)),
        height: Math.min(100, Math.max(0, Number(issue.zone?.height) || 10)),
      },
    })),
