  ...
  const shortcutDefs = loadShortcuts().map((entry) => {
    switch (entry.id) {
      case "command-palette":
        return toShortcutDef(entry, () => setCommandPaletteOpen(true));
      case "shortcuts-panel":
        return toShortcutDef(entry, () => setShortcutsPanelOpen((p) => !p));
      case "new-audit":
        return toShortcutDef(entry, () => router.push("/audits/new"));
      case "new-campaign":
        return toShortcutDef(entry, () => router.push("/campaigns/new"));
      case "go-dashboard":
        return toShortcutDef(entry, () => router.push("/dashboard"));
      case "go-audits":
        return toShortcutDef(entry, () => router.push("/audits"));
      case "go-campaigns":
        return toShortcutDef(entry, () => router.push("/campaigns"));
      case "go-settings":
        return toShortcutDef(entry, () => router.push("/settings"));
      default:
        return null;
    }
  }).filter(Boolean) as {
      ...
      {leaderKey && (
        <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 rounded-full bg-neutral-900 px-4 py-2 text-sm text-white shadow-lg animate-in fade-in slide-in-from-bottom-2">
          <span className="opacity-60">
            Mode leader&nbsp;·&nbsp;appuyez sur
          </span>{" "}
          <kbd className="mx-0.5 rounded bg-white/20 px-1.5 py-0.5 font-mono text-xs uppercase">
            D
          </kbd>
          <kbd className="mx-0.5 rounded bg-white/20 px-1.5 py-0.5 font-mono text-xs uppercase">
            A
