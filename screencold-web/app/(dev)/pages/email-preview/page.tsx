...
import { postSignupSequence, renderEmail } from "@/lib/email-sequences";
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-16">
      <div className="mb-10">
        ...
        <h1 className="mt-4 text-3xl font-bold tracking-tight text-white">Cold Email Templates</h1>
            ...
            >
              <div className="flex items-center justify-between border-b border-[#1f2937] px-5 py-3">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-white">Day {tpl.delayDays}</span>
                </div>
                <span className="text-xs text-gray-500 font-mono">+{tpl.delayDays}d delay</span>
              </div>

              <div className="px-5 py-4">
                <p className="text-xs text-gray-500 mb-1">Subject</p>
                <p className="text-sm font-medium text-white mb-4">{email.subject}</p>
      ...
      <div className="mt-12 rounded-xl border border-[#1f2937] bg-[#111827] p-6">
        <h2 className="text-sm font-semibold text-white mb-3">Template Variables</h2>
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-[#1f2937]">
                <th className="pb-2 text-left text-gray-500 font-medium">Variable</th>
                <th className="pb-2 text-left text-gray-500 font-medium">Value</th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(sampleVars).map(([key, val]) => (
                <tr key={key} className="border-b border-[#1f2937]/50">
                  <td className="py-2 font-mono text-[#60a5fa]">{`{{${key}}}`}</td>
